...
interface DailyNoteData {
  ...
  assessmentConfirmed?: boolean; // true = doctor confirmed (via button or manual edit)
  assessmentWasAutoGenerated?: boolean; // flag set on auto-generate
  // Plan
  planItems: PlanItem[];
...
interface DailyProgressNoteProps {
  ...
  /** Admission date ISO string for clinical course summary */
...
export default function DailyProgressNote({
  ...
  // latestVisit and admissionDate used by parent/summary logic
  ...
  const admissionDayCount = useMemo(() => {
    ...
    return Math.floor((nowMs - admitMs) / 86_400_000);
  }, [admissionDate]);

  const [showCourseSummary, setShowCourseSummary] = useState(false);

  const courseSummaryNodes = useMemo(() => {
    if (!admissionDate || admissionDayCount < 7) return [];
    const nodes: Array<{
      date: string;
      dayNum: number;
      note: DailyNoteData | null;
    }> = [];
    for (let d = 0; d <= admissionDayCount; d++) {
      const dateMs = new Date(admissionDate).getTime() + d * 86_400_000;
      const dateStr = format(new Date(dateMs), "yyyy-MM-dd");
      const dayNote = loadNote(doctorEmail, patientId, dateStr);
      if (dayNote) {
        nodes.push({ date: dateStr, dayNum: d + 1, note: dayNote });
      }
    }
    return nodes;
  }, [admissionDate, admissionDayCount, doctorEmail, patientId]);

  // ── Assessment confirmation state ─────────────────────────────────────────
  const [assessmentConfirmedLocal, setAssessmentConfirmedLocal] = useState(
    () => note.assessmentConfirmed ?? false,
  );
  const [assessmentModified, setAssessmentModified] = useState(false);
  const showAssessmentWarning =
    (note.assessmentWasAutoGenerated ?? false) &&
    !assessmentConfirmedLocal &&
    !assessmentModified &&
    !!note.assessment;
  ...
  function generateAssessment() {
    ...
    updateNote({
      assessment: text,
      assessmentWasAutoGenerated: true,
      assessmentConfirmed: false,
    });
    setAssessmentConfirmedLocal(false);
    setAssessmentModified(false);
    toast.success("Assessment generated — please review before confirming");
  }
    ...
    <div className="space-y-4">
          ...
          >
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="text-xs text-purple-600 bg-purple-100 px-2 py-0.5 rounded-full">
                Day 1 – Day {admissionDayCount}
              </span>
              {courseSummaryNodes.length > 0 && (
                <span className="text-xs text-purple-500">
                  · {courseSummaryNodes.length} note
                  {courseSummaryNodes.length !== 1 ? "s" : ""} recorded
                </span>
              )}
            </div>
            {showCourseSummary ? (
            ...
            <div className="px-4 pb-4 border-t border-purple-200">
              {courseSummaryNodes.length === 0 ? (
                <p className="text-sm text-purple-500 text-center py-4">
                  No daily notes recorded yet for this admission.
                </p>
              ) : (
                <div className="relative mt-3">
                  <div className="absolute left-3.5 top-0 bottom-0 w-0.5 bg-purple-200" />
                  <div className="space-y-3">
                    {courseSummaryNodes.map((node) => {
                      const primaryDx = node.note?.activeDiagnoses.find(
                        (d) =>
                          d.classification === "Primary" &&
                          d.status === "active",
                      );
                      const complaints = node.note?.activeComplaints
                        .filter((c) => !c.resolved)
                        .map((c) => c.text)
                        .join(", ");
                      const planDrugs = node.note?.planItems
                        .filter((pi) => pi.category === "drug")
                      ...
                      return (
                        <div
                          ...
                          data-ocid={`daily_note.course_node.${node.dayNum}`}
                        >
                          <div className="w-7 h-7 rounded-full bg-purple-600 text-white flex items-center justify-center text-xs font-bold shrink-0 relative z-10">
                            {node.dayNum}
                          </div>
                          <div className="flex-1 bg-white border border-purple-200 rounded-xl p-3 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              <span className="text-xs font-bold text-purple-800">
                                Day {node.dayNum} —{" "}
                                {format(new Date(node.date), "d MMM yyyy")}
                              </span>
                              {node.note?.assessmentStatus && (
                                <span
                                  className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold border ${statusColor(node.note.assessmentStatus)}`}
                                >
                                  {node.note.assessmentStatus}
                                </span>
                              )}
      ...
      {activeTab === "assessment" && (
        <div className="bg-white rounded-xl border border-amber-200 shadow-sm p-5 space-y-4">
          ...
          {/* ── Auto-generation Warning Banner ── */}
          {showAssessmentWarning && canEdit && (
            <div
              className="flex items-start gap-3 bg-yellow-50 border border-yellow-300 rounded-xl p-3"
              data-ocid="daily_note.assessment_warning.banner"
            >
              <AlertTriangle className="w-4 h-4 text-yellow-600 shrink-0 mt-0.5" />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-yellow-800">
                  ⚠ Auto-generated from your S/O entries
                </p>
                <p className="text-xs text-yellow-700 mt-0.5">
                  This is a starting point, not a clinical record until you
              ...
              <div className="flex gap-2 shrink-0">
                <Button
                  ...
                  onClick={() => {
                    setAssessmentConfirmedLocal(true);
                    updateNote({ assessmentConfirmed: true });
                    toast.success("Assessment confirmed");
                  }}
                  data-ocid="daily_note.assessment_confirm_button"
                >
                  <CheckCircle2 className="w-3 h-3" /> Confirm as Written
                </Button>
                <Button
                  size="sm"
                  variant="outline"
                  className="h-7 px-2.5 text-xs border-yellow-400 text-yellow-800"
                  onClick={() => {
                  ...
                  data-ocid="daily_note.assessment_edit_button"
                >
                  Edit
                </Button>
              </div>
          ...
          {/* ── Modified & Confirmed badge ── */}
          {assessmentModified && !showAssessmentWarning && (
            <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-3 py-2">
              <CheckCircle2 className="w-4 h-4 text-green-600 shrink-0" />
              <span className="text-xs font-semibold text-green-700">
                Modified & Confirmed by doctor
              </span>
            </div>
          )}
          ...
          <Textarea
            ...
            onChange={(e) => {
              updateNote({
                ...
                assessmentWasAutoGenerated: false,
                assessmentConfirmed: true,
              });
              setAssessmentModified(true);
              setAssessmentConfirmedLocal(true);
            }}
          ...
          >
            ...
            {note.assessmentWasAutoGenerated && !note.assessmentConfirmed && (
